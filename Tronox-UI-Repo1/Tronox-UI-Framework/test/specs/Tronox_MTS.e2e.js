...
const DATA_FILE = path.join(__dirname, '../Data/Tronox/Physicalinventory.json');
...
async function takeScreenshot(stepName) {
    ...
    const filePath = path.join(SCREENSHOT_DIR, `screenshot_${stepName}_${Date.now()}.png`);
...
async function switchToNewWindow() {
    ...
    if (windows.length > 1) {
        ...
        console.error('New window did not open.');
...
async function loadDataset() {
    ...
    try {
        ...
        return await import(DATA_FILE, { assert: { type: 'json' } });
...
describe('WebDriverIO Test Suite', () => {
    ...
    it('should perform test actions', async () => {
